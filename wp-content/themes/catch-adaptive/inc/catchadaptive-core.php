...
if ( ! function_exists( 'catchadaptive_setup' ) ) :
	...
	function catchadaptive_setup() {
		 ...
		 * to change 'catchadaptive' to the name of your theme in all the template files
		 */
		load_theme_textdomain( 'catchadaptive', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'primary' 	=> __( 'Primary Menu', 'catchadaptive' )
		) );
 ...
 * Default Options.
 */
require get_template_directory() . '/inc/catchadaptive-default-options.php';

/**
 * Custom Header.
 */
require get_template_directory() . '/inc/catchadaptive-custom-header.php';


/**
 * Structure for catchadaptive
 */
require get_template_directory() . '/inc/catchadaptive-structure.php';


/**
 * Customizer additions.
 */
require get_template_directory() . '/inc/customizer-includes/catchadaptive-customizer.php';


/**
 * Custom Menus
 */
require get_template_directory() . '/inc/catchadaptive-menus.php';


/**
 * Load Slider file.
 */
require get_template_directory() . '/inc/catchadaptive-featured-slider.php';


/**
 * Load Featured Content.
 */
require get_template_directory() . '/inc/catchadaptive-featured-content.php';


/**
 * Load Breadcrumb file.
 */
require get_template_directory() . '/inc/catchadaptive-breadcrumb.php';


/**
 * Load Widgets and Sidebars
 */
require get_template_directory() . '/inc/catchadaptive-widgets.php';


/**
 * Load Social Icons
 */
require get_template_directory() . '/inc/catchadaptive-social-icons.php';


/**
 * Load Metaboxes
 */
require get_template_directory() . '/inc/catchadaptive-metabox.php';
...
function catchadaptive_get_theme_options() {
	return get_theme_mod( 'catchadaptive_theme_options', catchadaptive_get_default_theme_options() );
}
	...
	function catchadaptive_content_nav( $nav_id ) {
		...
		?>
	        <nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>">
	        	<h3 class="screen-reader-text"><?php _e( 'Post navigation', 'catchadaptive' ); ?></h3>
				<?php
				/**
				...
				if ( 'numeric' == $pagination_type && function_exists( 'wp_pagenavi' ) ) {
	                ...
	                <div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'catchadaptive' ) ); ?></div>
	                <div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'catchadaptive' ) ); ?></div>
	            <?php 
	            } ?>
	        </nav><!-- #nav -->	
		<?php
	...
	function catchadaptive_comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div class="comment-body">
				<?php _e( 'Pingback:', 'catchadaptive' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'catchadaptive' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
			...
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				<footer class="comment-meta">
					<div class="comment-author vcard">
						...
						<?php printf( __( '%s <span class="says">says:</span>', 'catchadaptive' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
					</div><!-- .comment-author -->

					<div class="comment-metadata">
						<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
							<time datetime="<?php comment_time( 'c' ); ?>">
								<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'catchadaptive' ), get_comment_date(), get_comment_time() ); ?>
							</time>
						</a>
						<?php edit_comment_link( __( 'Edit', 'catchadaptive' ), '<span class="edit-link">', '</span>' ); ?>
					</div><!-- .comment-metadata -->

					<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'catchadaptive' ); ?></p>
					<?php endif; ?>
				</footer><!-- .comment-meta -->
...
if ( ! function_exists( 'catchadaptive_entry_meta' ) ) :
	...
	function catchadaptive_entry_meta() {
		...
		printf( '<span class="posted-on">%1$s<a href="%2$s" rel="bookmark">%3$s</a></span>',
			sprintf( _x( '<span class="screen-reader-text">Posted on</span>', 'Used before publish date.', 'catchadaptive' ) ),
		...
		if ( is_singular() || is_multi_author() ) {
			printf( '<span class="byline"><span class="author vcard">%1$s<a class="url fn n" href="%2$s">%3$s</a></span></span>',
				sprintf( _x( '<span class="screen-reader-text">Author</span>', 'Used before post author name.', 'catchadaptive' ) ),
		...
		if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) {
			...
			comments_popup_link( esc_html__( 'Leave a comment', 'catchadaptive' ), esc_html__( '1 Comment', 'catchadaptive' ), esc_html__( '% Comments', 'catchadaptive' ) );
			echo '</span>';
		}

		edit_post_link( esc_html__( 'Edit', 'catchadaptive' ), '<span class="edit-link">', '</span>' ); 
...
if ( ! function_exists( 'catchadaptive_tag_category' ) ) :
	...
	function catchadaptive_tag_category() {
		...
		if ( 'post' == get_post_type() ) {
			$categories_list = get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'catchadaptive' ) );
			if ( $categories_list && catchadaptive_categorized_blog() ) {
				printf( '<span class="cat-links">%1$s%2$s</span>',
					sprintf( _x( '<span class="screen-reader-text">Categories</span>', 'Used before category names.', 'catchadaptive' ) ),
					$categories_list
				);
			}

			$tags_list = get_the_tag_list( '', _x( ', ', 'Used between list items, there is a space after the comma.', 'catchadaptive' ) );
			if ( $tags_list ) {
				printf( '<span class="tags-links">%1$s%2$s</span>',
					sprintf( _x( '<span class="screen-reader-text">Tags</span>', 'Used before tag names.', 'catchadaptive' ) ),
					$tags_list
				);
			}
		}
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	...
	*/
	function catchadaptive_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
	...
	add_filter( 'wp_title', 'catchadaptive_wp_title', 10, 2 );
	...
	*/
	function catchadaptive_render_title() {
	?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
	<?php
	}
	add_action( 'wp_head', 'catchadaptive_render_title' );
endif;
...
if ( ! function_exists( 'catchadaptive_continue_reading' ) ) :
	...
	function catchadaptive_continue_reading() {
		...
		return ' <a class="more-link" href="' . esc_url( get_permalink() ) . '">' .  sprintf( __( '%s', 'catchadaptive' ) , $more_tag_text ) . '</a>';
	}
...
if ( ! function_exists( 'catchadaptive_body_classes' ) ) :
	...
	function catchadaptive_body_classes( $classes ) {
		...
		if ( $page_id == $page_for_posts || $page_id == $page_on_front ) {
	        $layout = get_post_meta( $page_id,'catchadaptive-layout-option', true );
	    }
	    else if ( is_singular() ) {
	 		if ( is_attachment() ) { 
		...
		if ( has_post_thumbnail() && 'full-content' != $featured_image ) { ?>
			<figure class="featured-image">
	            <a rel="bookmark" href="<?php the_permalink(); ?>">
	                <?php 
						if ( $featured_image == 'excerpt-image-left' ) {
		                    the_post_thumbnail( 'catchadaptive-featured-content' );
		                }
		                elseif ( $featured_image == 'columns-layout' ) {         	
							if ( is_sticky() ) {
...
if ( ! function_exists( 'catchadaptive_scrollup' ) ) {
	...
	function catchadaptive_scrollup() {
		...
		if ( !$catchadaptive_scrollup = get_transient( 'catchadaptive_scrollup' ) ) {
			...
			$catchadaptive_scrollup =  '<a href="#masthead" id="scrollup" class="genericon"><span class="screen-reader-text">' . __( 'Scroll Up', 'catchadaptive' ) . '</span></a>' ;
...
if ( ! function_exists( 'catchadaptive_page_post_meta' ) ) :
	...
	function catchadaptive_page_post_meta() {
		...
		$catchadaptive_page_post_meta = '<span class="post-time">' . __( 'Posted on', 'catchadaptive' ) . ' <time class="entry-date updated" datetime="' . esc_attr( get_the_date( 'c' ) ) . '" pubdate>' . esc_html( get_the_date() ) . '</time></span>';
	    $catchadaptive_page_post_meta .= '<span class="post-author">' . __( 'By', 'catchadaptive' ) . ' <span class="author vcard"><a class="url fn n" href="' . $catchadaptive_author_url . '" title="View all posts by ' . get_the_author() . '" rel="author">' .get_the_author() . '</a></span>';
...
if ( ! function_exists( 'catchadaptive_post_navigation' ) ) :
	...
	function catchadaptive_post_navigation() {
		...
		the_post_navigation( array(
			'next_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Next &rarr;', 'catchadaptive' ) . '</span> ' .
				'<span class="screen-reader-text">' . __( 'Next post:', 'catchadaptive' ) . '</span> ' .
				'<span class="post-title">%title</span>',
			'prev_text' => '<span class="meta-nav" aria-hidden="true">' . __( '&larr; Previous', 'catchadaptive' ) . '</span> ' .
				'<span class="screen-reader-text">' . __( 'Previous post:', 'catchadaptive' ) . '</span> ' .
				'<span class="post-title">%title</span>',
		) );
