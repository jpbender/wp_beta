...
if( !function_exists( 'catchadaptive_custom_breadcrumbs' ) ) :
	...
	function catchadaptive_custom_breadcrumbs( $showOnHome, $delimiter ) {
		...
		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'catchadaptive' ); // text for the 'Home' link
		$text['category'] = __( '%1$s Archive for %2$s', 'catchadaptive' ); // text for a category page
		$text['search']   = __( '%1$sSearch results for: %2$s', 'catchadaptive' ); // text for a search results page
		$text['tag']      = __( '%1$sPosts tagged %2$s', 'catchadaptive' ); // text for a tag page
		$text['author']   = __( '%1$sView all posts by %2$s', 'catchadaptive' ); // text for an author page
		$text['404']      = __( 'Error 404', 'catchadaptive' ); // text for the 404 page
		...
		if( is_front_page() ) {
			...
			if( get_query_var( 'paged' ) ) {
				...
				echo sprintf( __( 'Page %s', 'catchadaptive' ), max( $paged, $page ) );
				if( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
