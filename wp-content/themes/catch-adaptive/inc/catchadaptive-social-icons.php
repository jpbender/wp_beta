	...
	if( ( !$catchadaptive_social_icons = get_transient( 'catchadaptive_social_icons' ) ) ) {
		...
		foreach ( $social_icons as $key => $value ) {
			if( '' != $value ){
				$title	=	explode( '-', $key );
				if ( 'Mail' == $key  ) { 
					$output .= '<a class="genericon_parent genericon genericon-'. esc_attr( strtolower( $key ) ) .'" title="'. __( 'Email', 'catchadaptive') . '" href="mailto:'. sanitize_email( $value ) .'"><span class="screen-reader-text">'. __( 'Email', 'catchadaptive') . '</span> </a>';
				}
				else if ( 'Skype' == $key  ) { 
