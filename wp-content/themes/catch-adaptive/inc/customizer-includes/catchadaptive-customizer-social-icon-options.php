...
if ( ! defined( 'CATCHADAPTIVE_THEME_VERSION' ) ) {
	...
	if( 4 <= get_bloginfo( 'version' ) ) {
		$wp_customize->add_panel( 'catchadaptive_social_links', array(
		    ...
		    'description'	=> __( 'Note: Enter the url for correponding social networking website', 'catchadaptive' ),
		    'priority'       => 600,
			'title'    		 => __( 'Social Links', 'catchadaptive' ),
		) );
	}
	...
	foreach ( $catchadaptive_social_icons as $option ){
		...
		if( $option == 'Skype' ){
			...
			$wp_customize->add_control( 'catchadaptive_'. $lower_case_option .'_link', array(
				'description'	=> __( 'Skype link can be of formats:<br>callto://+{number}<br> skype:{username}?{action}. More Information in readme file', 'catchadaptive' ),
				'label'    		=> $option,
				'priority' 		=> $i + '2',
