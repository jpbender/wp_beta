...
function catchadaptive_widgets_init() {
	...
	register_sidebar( array(
		'name'          => __( 'Primary Sidebar', 'catchadaptive' ),
		'id'            => 'primary-sidebar',
		'before_widget' => '<section id="%1$s" class="widget %2$s"><div class="widget-wrap">',
		'after_widget'  => '</div><!-- .widget-wrap --></section><!-- .widget -->',
		'before_title'  => '<h4 class="widget-title">',
		'after_title'   => '</h4>',
		'description'	=> __( 'This is the primary sidebar if you are using two column site layout option.', 'catchadaptive' ),
	) );
	...
	for( $i=1; $i <= $footer_sidebar_number; $i++ ) {
		register_sidebar( array(
			'name'          => sprintf( __( 'Footer Area %d', 'catchadaptive' ), $i ),
			'id'            => sprintf( 'footer-%d', $i ),
			'before_widget' => '<section id="%1$s" class="widget %2$s"><div class="widget-wrap">',
			'after_widget'  => '</div><!-- .widget-wrap --></section><!-- #widget-default-search -->',
			'before_title'  => '<h4 class="widget-title">',
			'after_title'   => '</h4>',
			'description'	=> sprintf( __( 'Footer %d widget area.', 'catchadaptive' ), $i ),
		) );
	}
}
...
class Catchadaptive_social_icons_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			__( 'CT: Social Icons', 'catchadaptive' ), // Name
			array( 'description' => __( 'Use this widget to add Social Icons as a widget. ', 'catchadaptive' ) ) // Args
		);
	}
	...
	public function form( $instance ) {
		...
		?>
		<p>
		<label for="<?php echo $this->get_field_name( 'title' ); ?>"><?php _e( 'Title (optional):', 'catchadaptive' ); ?></label>
